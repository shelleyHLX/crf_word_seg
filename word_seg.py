...
sents = open('./icwb2-data/training/msr_training.utf8').readlines()

# exit(0)
print(len(sents))  # 86924
print(type(sents))  # <class 'list'>
sents = [sent.strip() for sent in sents]
print(len(sents))  # 86924
sents = [sent.split(' ') for sent in sents]  # 这个语料的换行符是\r\n
print(len(sents))    # 86924
print(type(sents))  # <class 'list'>
print(''.join(sents[0]))
...
print(sents[0])  # ['记者', '：', '你们', '见到', '桑兰', '时', '说', '了', '些', '什么', '？']
print(len(sents))  # 86924
print(np.array(sents).shape)  # (86924,)
# exit(0)
...
# train_sents = sents[:50]  # 留下5000个句子做验证，剩下的都用来训练
# valid_sents = sents[50:100]
...
def train_generator():  # 定义数据生成器
    while True:
        X,Y = [],[]
        for i,s in enumerate(train_sents):  # 遍历每个句子  # ['记者', '：', '你们', '见到', '桑兰', '时', '说', '了', '些', '什么', '？']
            sx,sy = [],[]
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if acc > self.highest:
            self.highest = acc
            model.save('word_seg_{}.h5'.format(epoch))
...
if __name__ == '__main__':
    # model.load_weights('word_seg_9.h5')
    # train()

    model.load_weights('word_seg_5.h5')
    predi()
...
_________________________________________________________________
embedding_1 (Embedding)      (None, None, 128)         606208    
_________________________________________________________________
